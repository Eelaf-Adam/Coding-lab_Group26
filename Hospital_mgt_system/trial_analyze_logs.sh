...
#Assigning variables
choices=("Heart Rate" "Temperature" "Water Usage")
time_stamp=$(date '+%Y-%m-%d %H:%M:%S')
log_files=("heart_rate.log" "temperature_rate.log" "water_usage.log")
...
#The command will extract the ten line from the head of the log file selected and filter the first two sections which include date and time
...
#Setting variable for device counting

device_counter=$(awk '{print $3}' "$log_files" | sort | uniq -c | sort -nr)
#The command will filter out the third row of the log file counting temperature recorders (A&B) sort in order and count them
