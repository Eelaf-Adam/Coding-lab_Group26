...
case $log_selected in
	...
	2)
		...
		archived_logs_filename="temperature_${timestamp}.log"
		#Moving temperature log file to archive logs directory and storing it in a new file with time stamp included in file name
		mv "${log_files[1]}" "${archive_logs[1]}/${archived_logs_filename}"
		echo "'${log_files[1]} transfered to '${archive_logs[1]}'"
		#Creation of file for continued heart rate monitoring
		touch temperature_monitor
		;;
	3)
		e
